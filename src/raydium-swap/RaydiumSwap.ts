 ...
 */
  findPoolInfoForTokens(mintA: string, mintB: string) {
    ...
    fs.writeFileSync(filePath, JSON.stringify(poolData));
 ...
 */
  async getSwapTransaction(
    ...
    const [{ amountOut ,minAmountOut, amountIn }, userTokenAccounts, recentBlockhashForSwap] = await Promise.all([this.calcAmountOut(poolKeys, amount, directionIn), this.getOwnerTokenAccounts(), this.connection.getLatestBlockhash()]);
 ...
 */
  ...
  async transactionConfirmation(txid: string): Promise<void> {
    let subscriptionId;
    try {
